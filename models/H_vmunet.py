...
class H_vmunet(nn.Module):
    ...
    def __init__(self, num_classes=2, input_channels=3,layer_scale_init_value=1e-6,H_SS2D=H_SS2D, block=Block,pretrained=None,
...
'''       
    ...
    def forward(self, x1,x2):
        out1 = F.gelu(F.max_pool2d(self.ebn1(self.encoder1(x1)),2,2))
        t1_1 = out1 # b, c0, H/2, W/2
        out1 = F.gelu(F.max_pool2d(self.ebn2(self.encoder2(out1)),2,2))
        t2_1 = out1 # b, c1, H/4, W/4 
        out1 = F.gelu(F.max_pool2d(self.ebn3(self.encoder3(out1)),2,2))
        t3_1 = out1 # b, c2, H/8, W/8
        out1 = F.gelu(F.max_pool2d(self.ebn4(self.encoder4(out1)),2,2))
        t4_1 = out1 # b, c3, H/16, W/16
        out1 = F.gelu(F.max_pool2d(self.ebn5(self.encoder5(out1)),2,2))
        t5_1 = out1 # b, c4, H/32, W/32
        
        out2 = F.gelu(F.max_pool2d(self.ebn1(self.encoder1(x2)),2,2))
        t1_2 = out2 # b, c0, H/2, W/2
        out2 = F.gelu(F.max_pool2d(self.ebn2(self.encoder2(out2)),2,2))
        t2_2 = out2 # b, c1, H/4, W/4 
        out2 = F.gelu(F.max_pool2d(self.ebn3(self.encoder3(out2)),2,2))
        t3_2 = out2 # b, c2, H/8, W/8  
        out2 = F.gelu(F.max_pool2d(self.ebn4(self.encoder4(out2)),2,2))
        t4_2 = out2 # b, c3, H/16, W/16
        out2 = F.gelu(F.max_pool2d(self.ebn5(self.encoder5(out2)),2,2))
        t5_2 = out2 # b, c4, H/32, W/32
        
        print('t1_1',t1_1.shape)
        print('t2_1',t2_1.shape)
        print('t3_1',t3_1.shape)
        print('t4_1',t4_1.shape)
        print('t5_1',t5_1.shape)
        exit()
        
        t1 = self.conv(torch.cat([t1_1,t1_2],dim=1))
        t2 = self.conv(torch.cat([t2_1,t2_2],dim=1))
        t3 = self.conv(torch.cat([t3_1,t3_2],dim=1))
        t4 = self.conv(torch.cat([t4_1,t4_2],dim=1))
        t5 = self.conv(torch.cat([t5_1,t5_2],dim=1))
        
        if self.bridge: t1, t2, t3, t4, t5 = self.scab(t1, t2, t3, t4, t5)

        out1 = F.gelu(self.encoder6(out1))
        out2 = F.gelu(self.encoder6(out2))        # b, c5, H/32, W/32
