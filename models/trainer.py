...
from models.losses import mixed_loss
...
class CDTrainer():
    ...
    def _forward_pass(self, batch):
        ...
        self.G_final_pred = self.G_pred1

            
    def _backward_G(self):
        gt = self.batch['L'].to(self.device).float()
        self.G_loss = self._pxl_loss(self.G_pred1, gt)+self._pxl_loss(self.G_pred2, gt)+self._pxl_loss(self.G_pred3, gt)+self._pxl_loss(self.G_pred4, gt)
