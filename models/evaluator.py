...
class CDEvaluator():
    ...
    def _collect_running_batch_states(self):
        ...
        if np.mod(self.batch_id, 1) == 0:
            ...
            vis_input = np.clip(vis_input, a_min=0.0, a_max=1.0)
            vis_input2 = np.clip(vis_input2, a_min=0.0, a_max=1.0)
            ...
            input1_dir = os.path.join(self.vis_dir, 'input1')
            input2_dir = os.path.join(self.vis_dir, 'input2')
            pred_dir = os.path.join(self.vis_dir, 'predictions')
            gt_dir = os.path.join(self.vis_dir, 'ground_truth')
            ...
            # Define file names
            file_name_input1 = os.path.join(input1_dir, f'eval_{self.batch_id}.jpg')
            file_name_input2 = os.path.join(input2_dir, f'eval_{self.batch_id}.jpg')
            file_name_pred = os.path.join(pred_dir, f'eval_{self.batch_id}.jpg')
            file_name_gt = os.path.join(gt_dir, f'eval_{self.batch_id}.jpg')

            # Save images to the corresponding folders
            plt.imsave(file_name_input1, vis_input)
            plt.imsave(file_name_input2, vis_input2)
    ...
    def _forward_pass(self, batch):
        ...
        self.G_pred1,self.G_pred2,self.G_pred3,self.G_pred4 = self.net_G(img_in1, img_in2)
